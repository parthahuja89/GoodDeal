...
export default function GameSearchPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-12  text-gray-100 min-h-screen">
      ...
      {searchQuery && (
        <div className="absolute z-50 w-full max-w-2xl bg-slate-800 rounded-lg shadow-lg mt-2 max-h-96 overflow-y-auto">
          <SearchResults />
          <button
        onClick={() => setSearchQuery("")}
        className="absolute top-2 right-2 p-2 hover:bg-slate-700 rounded-full"
          >
        <X className="h-4 w-4" />
          </button>
        </div>
