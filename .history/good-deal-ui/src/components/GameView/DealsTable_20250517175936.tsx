...
export default function DealsTable(deals: GameDeal[]) {
  const [data, setData] = useState<GamePrice[]>(gameData)
  ...
  // Add this sorting function before the filtering logic
  ...
  // Update the filteredData variable to include sorting
          ...
          <TableBody>
            {filteredData.length > 0 ? (
              filteredData.map((item) => (
                <TableRow key={item.id} className="hover:bg-gray-800 cursor-pointer">
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-2">
                      <div>
                        <div className="font-semibold">
                          {item.store} {item.region && item.region}
                        </div>
                  ...
                  <TableCell className="text-right">
                    <div className="text-amber-500 font-bold">${item.storeLowPrice.toFixed(2)}</div>
                    <div className="text-sm text-gray-500">
                      {item.isPriceBetter ? "better" : "worse"} by ${item.priceDifference.toFixed(2)}
                    </div>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex flex-col items-end">
                      ...
                      <div className="text-sm text-gray-500">${item.originalPrice.toFixed(2)}</div>
                    </div>
                  </TableCell>
                </TableRow>
              ))
