        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex flex-col justify-between items-start">
              <div
                className="relative bg-blue-300 rounded-lg overflow-hidden w-full h-48 bg-opacity-30"
                style={{
          ...
          <CardContent>
            <div>
              <p className="text-sm">
                <strong>Release Date:</strong> {game?.release_date ? new Date(game.release_date).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }) : ''}
              </p>
              <p className="text-sm">
                <strong>Developers:</strong> {game?.developers.map((dev, index) => (
                  <span key={index}>
                    {dev.name}
