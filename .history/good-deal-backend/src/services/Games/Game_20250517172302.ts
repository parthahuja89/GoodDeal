...
export const getGamePrices = async (gameId: string): Promise<GameDeal[]> => {
  try {
    const response = await axios.post(`${BASE_URL}/prices/v3`, {
      key: ITAD_API_KEY,
      id: gameId,
    });
    const data = response.data;
    if (data.error) {
      logger.error("Error fetching game prices:", data.error);
      throw new Error(data.error);
    }
    const gameDeals: GameDeal[] = data.data.map((deal: any) => ({
      id: deal.id,
      asset_url: deal.assets?.banner600 || deal.assets?.banner400,
      title: deal.title,
      price_regular: deal.regularPrice,
      price_new: deal.newPrice,
      url: deal.url,
      store: deal.store,
    }));
    return gameDeals;
  } catch (error) {
    logger.error("Error fetching game prices:", error);
    throw error;
  }
}