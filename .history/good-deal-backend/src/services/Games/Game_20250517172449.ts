...
export const getGamePrices = async (gameId: string): Promise<GameDeal[]> => {
  try {
    const response = await axios.get(`${BASE_URL}/games/prices/v3`, {
      params: {
      key: ITAD_API_KEY,
      id: gameId
      },
    ...
    if (data.error) {
      logger.error("Error fetching game deals:", data.error, data.error.message);
      throw new Error(data.error);
    }

    const deals: GameDeal[] = data.list.map((deal: any) => ({
      title: deal.title,
      id: deal.id,
      asset_url: deal.assets?.banner600 || deal.assets?.banner400,
      price_new: deal.deal.price.amount,
      price_regular: deal.deal.regular.amount,
      store: deal.store,
      url: deal.deal.url,
    }));
